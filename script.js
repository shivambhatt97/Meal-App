//make a favrouid meal array if this is not exist in local storage
if(localStorage.getItem("favouritesList") == null){
    localStorage.setItem("favouritesList",JSON.stringify([]))
}


//fetch meal from api and return it
...
function showMealList(){
    let inputVal=document.getElementById("my-search").value;
    let arr=JSON.parse(localStorage.getItem("favouritesList"));
    let url="https://www.themealdb.com/api/json/v1/1/search.php?s=";
    let html ="";
    let meals=fetchMealFromApi(url,inputVal);
    meals.then(data=>{
        if(data.meals){
            data.meals.forEach((element) => {
                ...
                if(isFav){
                    ...
                    <button type="button" class="btn btn-outline-light" onclick="showMoreDetails(${element.idMeal})">More Details</button>
                    <button id="main${element.isMeal}" class="btn btn-outline-light active" onclick="addRemoveToFavList(${element.idMeal})"
...
async function showMealDetails(id){
    ...
    let html="";
    await fetchMealFromApi(url,id).then(data=>{
        html += `
...
//favourite meals in favorite list
async function favMealList(){
    ...
    if(arr.length == 0){
        ...
        <span display="display-1 d-block">404</span>
        <div class="mb-4 lead">
        ...
        for(let index=0;index<arr.length;index++){
            await fetchMealFromApi(url,arr[index]).then(data=>{
                html +=`
                <div id="card" class="card mb-3" style="width:20rem;">
...
function addRemoveToFavList(id){
    ...
    if(contain){
        ...
        arr.push(id);
        alert("Your meal add into favourite list");
    }
